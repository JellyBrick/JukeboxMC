...
abstract class EntityMoveable : Entity() {
    ...
    var isCollided = false
        protected set
    protected override var fallDistance = 0
    protected fun checkObstruction(x: Float, y: Float, z: Float) {
        if (location.world.getCollisionCubes(this, getBoundingBox(), false).size == 0) {
            return
        }
    ...
    protected fun move(velocity: Vector) {
        ...
        val movZ = velocity.z
        val list =
            this.world.getCollisionCubes(this, boundingBox.addCoordinates(velocity.x, velocity.y, velocity.z), false)
        if (list != null) {
            for (bb in list) {
                velocity.y = bb!!.calculateYOffset(boundingBox, velocity.y)
            }
            ...
            boundingBox.offset(movX, movY, movZ)
        }
    }
    ...
    private fun sendEntityMovePacket(location: Location, onGround: Boolean) {
        ...
        moveEntityAbsolutePacket.setRuntimeEntityId(entityId)
        ...
        moveEntityAbsolutePacket.setRotation(Vector3f.from(location.pitch, location.yaw, location.yaw))
        Server.Companion.getInstance().broadcastPacket(moveEntityAbsolutePacket)
    }
