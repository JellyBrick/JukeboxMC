...
abstract class EntityLiving : Entity() {
    ...
    fun applyResistanceEffectReduction(event: EntityDamageEvent, damage: Float): Float {
        if (event.getEntity() is EntityLiving) {
            val entityLiving = event.getEntity() as EntityLiving
            if (entityLiving.hasEffect(EffectType.RESISTANCE)) {
                val amplifier: Int = entityLiving.getEffect<Effect>(EffectType.RESISTANCE)!!.amplifier
    ...
    fun addEffect(effect: Effect) {
        ...
        if (this is Player) {
            ...
            player.playerConnection.sendPacket(mobEffectPacket)
        }
    ...
    fun setHeal(value: Float, cause: EntityHealEvent.Cause) {
        ...
        health = Math.min(20f, Math.max(0f, entityHealEvent.heal))
    }
