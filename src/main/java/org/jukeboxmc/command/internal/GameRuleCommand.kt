...
class GameRuleCommand : Command(CommandData.Companion.builder()
    .setParameters(arrayOf<CommandParameter>(
        CommandParameter(
            ...
            Arrays.stream(GameRule.values())
                ...
                .toList(),
            false),
        CommandParameter("value", listOf("true", "false"), false)
        ...
        CommandParameter(
            ...
            Arrays.stream(GameRule.values())
                ...
                .toList(),
            false),
        CommandParameter("value", CommandParamType.INT, false))).build()) {
    override fun execute(commandSender: CommandSender, command: String?, args: Array<String?>) {
        if (commandSender is Player) {
            if (args.size == 2) {
                val gameRuleValue = args[0]
                val gameRule: GameRule = GameRule.Companion.fromIdentifier(gameRuleValue)
                if (gameRule != null) {
