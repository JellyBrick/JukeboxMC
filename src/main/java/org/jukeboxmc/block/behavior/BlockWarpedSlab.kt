        ...
        if (blockFace == BlockFace.DOWN) {
            if (targetBlock is BlockWarpedSlab) {
                if (targetBlock.isTopSlot) {
                    world.setBlock(blockPosition, Block.Companion.create<Block>(BlockType.WARPED_DOUBLE_SLAB))
                    return true
                }
            } else if (block is BlockWarpedSlab) {
                world.setBlock(placePosition, Block.Companion.create<Block>(BlockType.WARPED_DOUBLE_SLAB))
                return true
            }
