...
class BlockLava : BlockLiquid {
    ...
    override fun checkForHarden() {
        ...
        if (colliding != null) {
            if (this.liquidDepth == 0) {
                liquidCollide(Block.Companion.create<Block>(BlockType.OBSIDIAN))
            } else if (this.liquidDepth <= 4) {
                liquidCollide(Block.Companion.create<Block>(BlockType.COBBLESTONE))
            }
        }
    }
    ...
    override fun flowIntoBlock(block: Block, newFlowDecay: Int) {
        if (block is BlockWater) {
            (block as BlockLiquid).liquidCollide(Block.Companion.create<Block>(BlockType.STONE))
        } else {
    ...
    override fun getBlock(liquidDepth: Int): BlockLiquid {
        val blockLava: BlockLava = Block.Companion.create<BlockLava>(BlockType.LAVA)
        blockLava.liquidDepth = liquidDepth
        return blockLava
    }
}