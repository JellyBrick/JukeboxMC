 ...
 * @version 1.0
 */
class BlockCutCopperSlab : BlockSlab {
        ...
        if (blockFace == BlockFace.DOWN) {
            if (targetBlock is BlockCutCopperSlab) {
                if (targetBlock.isTopSlot) {
                    world.setBlock(blockPosition, Block.Companion.create<Block>(BlockType.DOUBLE_CUT_COPPER_SLAB))
                    return true
                }
            } else if (block is BlockCutCopperSlab) {
                world.setBlock(placePosition, Block.Companion.create<Block>(BlockType.DOUBLE_CUT_COPPER_SLAB))
                return true
            }
