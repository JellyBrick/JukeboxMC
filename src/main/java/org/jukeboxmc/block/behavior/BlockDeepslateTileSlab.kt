 ...
 * @version 1.0
 */
class BlockDeepslateTileSlab : BlockSlab {
        ...
        if (blockFace == BlockFace.DOWN) {
            if (targetBlock is BlockDeepslateTileSlab) {
                if (targetBlock.isTopSlot) {
                    world.setBlock(blockPosition, Block.Companion.create<Block>(BlockType.DEEPSLATE_TILE_DOUBLE_SLAB))
                    return true
                }
            } else if (block is BlockDeepslateTileSlab) {
                world.setBlock(placePosition, Block.Companion.create<Block>(BlockType.DEEPSLATE_TILE_DOUBLE_SLAB))
                return true
            }
