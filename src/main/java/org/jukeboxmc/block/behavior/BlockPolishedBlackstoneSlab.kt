 ...
 * @version 1.0
 */
class BlockPolishedBlackstoneSlab : BlockSlab {
    ...
    override fun placeBlock(
        ...
        if (blockFace == BlockFace.DOWN) {
            if (targetBlock is BlockPolishedBlackstoneSlab) {
                if (targetBlock.isTopSlot) {
                    world.setBlock(
                        blockPosition,
                        create(BlockType.POLISHED_BLACKSTONE_DOUBLE_SLAB),
                    )
                    return true
                }
            } else if (block is BlockPolishedBlackstoneSlab) {
                world.setBlock(placePosition, create(BlockType.POLISHED_BLACKSTONE_DOUBLE_SLAB))
                return true
            }
