 ...
 * @version 1.0
 */
class BlockMudBrickSlab : BlockSlab {
        ...
        if (blockFace == BlockFace.DOWN) {
            if (targetBlock is BlockMudBrickSlab) {
                if (targetBlock.isTopSlot) {
                    world.setBlock(blockPosition, Block.Companion.create<Block>(BlockType.MUD_BRICK_DOUBLE_SLAB))
                    return true
                }
            } else if (block is BlockMudBrickSlab) {
                world.setBlock(placePosition, Block.Companion.create<Block>(BlockType.MUD_BRICK_DOUBLE_SLAB))
                return true
            }
