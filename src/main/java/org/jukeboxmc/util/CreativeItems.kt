    ...
    fun init() {
        ...
        try {
            Objects.requireNonNull<InputStream>(
                Bootstrap::class.java.getClassLoader().getResourceAsStream("creative_items.json")
            ).use { inputStream ->
                ...
                for (itemEntry in itemEntries["items"]!!) {
                    ...
                    val identifier: Identifier = Identifier.Companion.fromString((itemEntry["id"] as String?)!!)
                    ...
                    if (itemEntry.containsKey("damage")) {
                        item.damage((itemEntry["damage"] as Double).toShort().toInt())
                    }
