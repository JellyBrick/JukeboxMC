    ...
    override fun handle(packet: NpcRequestPacket, server: Server, player: Player) {
        val form = player.openNpcDialogueForms.stream()
            .filter { npcDialogueForm: NpcDialogueForm? ->
                npcDialogueForm!!.sceneName().equals(packet.getSceneName(), ignoreCase = true)
            }
            .findAny().orElse(null) ?: return
        val requestType: NpcRequestType = packet.getRequestType()
        ...
        val button = form.dialogueButtons()[packet.getActionType()] ?: return
